...
<body>
  ...
  app.get("/api/portfolio/balances", authenticate, async (req: any, res) => {
    try {
              ...
              if (balance.total > 0.001) {
                ...
                if (
                  ...
                  try {
                    ...
                    try {
                      const btcTicker = await exchangeService.getTicker(
                        userId,
                        apiKey.exchange,
                        `${balance.symbol}/BTC`
                      );
                      const btcUsdTicker = await exchangeService.getTicker(
                        userId,
                        apiKey.exchange,
                        "BTC/USDT"
                      );
      ...
      allBalances = allBalances.map((balance) => ({
        ...
        percentage:
          totalUSD > 0
            ? parseFloat(((balance.usdValue / totalUSD) * 100).toFixed(2))
            : 0,
      }));
    ...
    try {
            ...
            try {
              ...
              if (analysis) {
                ...
                const symbolSentiment =
                  momentumSentiment + trendSentiment + volatilitySentiment + profitSentiment;
      ...
      if (validAnalyses === 0) {
        return res.status(503).json({
          error: "LIVE_DATA_REQUIRED",
          message:
            "Market sentiment calculation failed. Exchange connections required for live analysis.",
        });
      }
  ...
  app.get("/api/market/price-prediction", async (req, res) => {
    try {
      ...
      const btc =
        tickers.find((t) => t.symbol === "BTC/USDT") ||
        tickers.find((t) => t.symbol.includes("BTC")) ||
        tickers[0];
      ...
      const prediction = {
        ...
        analysis: {
          ...
          rsiValue:
            change24h > 0
              ? Math.min(80, 50 + change24h * 5)
              : Math.max(20, 50 + change24h * 5),
  ...
  app.get("/api/trading-stats", authenticate, async (req, res) => {
    try {
      ...
      const profitableTrades = completedTrades.filter(
        (trade) => parseFloat(trade.pnl || "0") > 0
      );
      const winRate =
  ...
  app.get("/api/market/overview", async (req, res) => {
    ...
    if (tickers.length === 0) {
      ...
      return res.status(503).json({
        error: "LIVE_DATA_REQUIRED",
        message:
          "Live market data temporarily unavailable. Connect exchange API keys to restore live data.",
        timestamp: Date.now(),
      });
    }
    ...
    const btc =
      ...
      tickers[0];
    const eth =
      tickers.find((t) => t.symbol === "ETH/USDT") ||
      tickers.find((t) => t.symbol.includes("ETH")) ||
      tickers[1];
    const sol =
      tickers.find((t) => t.symbol === "SOL/USDT") ||
      tickers.find((t) => t.symbol.includes("SOL")) ||
      tickers[2];
  ...
  app.get("/api/portfolio/summary", authenticate, async (req: any, res) => {
    try {
      ...
      const response = {
        ...
        dayPnlPercent:
          totalUSDValue > 0 ? ((dayPnl / totalUSDValue) * 100).toFixed(2) : "0.00",
  ...
  app.post("/api/exchange/api-keys", authenticate, async (req: any, res) => {
    try {
      ...
      if (!exchange || !apiKey || !secretKey) {
        return res
          .status(400)
          .json({ error: "Exchange, API key, and secret key are required" });
      }
  ...
  app.post(
    "/api/exchange/test-connection/:id",
    authenticate,
    async (req: any, res) => {
      try {
        const userId = req.user.id;
  ...
  app.post("/api/exchange/test-direct", authenticate, async (req: any, res) => {
    try {
      ...
      if (!apiKey || !secretKey || !exchangeName) {
        return res
          .status(400)
          .json({ error: "API key, secret, and exchange are required" });
      }
      ...
      if (exchangeName === "kucoin") {
        exchange = new (ccxtLib as any).kucoin({
          apiKey: apiKey,
          secret: secretKey,
          sandbox: false,
        ...
        exchange = new (ccxtLib as any).bybit({
          apiKey: apiKey,
          secret: secretKey,
          sandbox: false,
          enableRateLimit: true,
        ...
        exchange = new (ccxtLib as any).binance({
          apiKey: apiKey,
          secret: secretKey,
          sandbox: false,
          enableRateLimit: true,
  ...
  app.post("/api/exchange/connect", authenticate, async (req: any, res) => {
    try {
      ...
      const encryptedPassphrase = passphrase
        ? exchangeService.encryptApiKey(passphrase)
        : null;
  ...
  app.post("/api/exchange/order", authenticate, async (req: any, res) => {
    try {
      ...
      if (!exchange || !symbol || !type || !side || !amount) {
        return res
          .status(400)
          .json({ error: "Exchange, symbol, type, side, and amount are required" });
      }
      ...
      await storage.createOrderAlert({
        ...
        type: "order",
        message: `${side.toUpperCase()} order placed for ${amount} ${symbol} at ${
          price || "market price"
        }`,
      });
    ...
    try {
      ...
      if (tickers.length > 0) {
        ...
        if (lowerMessage.includes("btc") || lowerMessage.includes("bitcoin")) {
          ...
          reply = `Bitcoin is currently trading at $${btcPrice.toLocaleString()} with a ${Math.abs(
            btcChange
          ).toFixed(2)}% ${trend} movement. Based on current market conditions, consider ${
            Math.abs(btcChange) > 2
              ? "momentum trading for volatility"
              : "DCA for accumulation"
          }. Key support at $${(btcPrice * 0.98).toFixed(0)}k.`;
          ...
          reply = `Ethereum is at $${ethPrice.toLocaleString()} with ${trend} sentiment (${ethChange.toFixed(
            2
          )}% change). ${
            Math.abs(ethChange) > 3
              ? "High volatility suggests grid trading opportunities."
              : "Stable conditions favor swing trading strategies."
          } Monitor gas fees for optimal entry.`;
        } else if (lowerMessage.includes("price") || lowerMessage.includes("prediction")) {
          const marketTrend = (btc?.percentage || 0) > 0 ? "bullish" : "bearish";
          reply = `Live market analysis shows ${marketTrend} conditions. BTC at $${btc?.last.toLocaleString()} (${btc?.percentage?.toFixed(
            2
          )}% change). Technical indicators suggest ${
            Math.abs(btc?.percentage || 0) > 1
              ? "increased volatility - use tight stop-losses"
              : "consolidation phase - good for range trading"
          }. Always manage risk appropriately.`;
        } else if (lowerMessage.includes("strategy")) {
          const volatility = Math.abs(btc?.percentage || 0);
          if (volatility > 3) {
            reply =
              "High market volatility detected. Recommended strategies: 1) Scalping for quick profits, 2) Grid trading with tight ranges, 3) Momentum trading with trailing stops. Use smaller position sizes in volatile conditions.";
          } else if (volatility > 1) {
            reply =
              "Moderate volatility present. Consider: 1) Swing trading on 4H timeframes, 2) DCA with weekly intervals, 3) Support/resistance trading. Good conditions for systematic approaches.";
          } else {
            reply =
              "Low volatility environment. Optimal for: 1) DCA accumulation strategies, 2) Long-term holding, 3) Wide grid trading setups. Perfect time for building positions gradually.";
          }
        } else {
        ...
        if (lowerMessage.includes("hello") || lowerMessage.includes("hi")) {
          reply =
            "Hello! I'm your AI trading assistant with live market analysis. Connect exchange APIs for real-time data-driven trading advice.";
        } else {
          reply =
            "I provide intelligent trading insights using live market data. Connect your exchange APIs (KuCoin, Bybit, Binance) for real-time analysis and personalized strategy recommendations.";
        }
      }
    ...
    try {
        ...
        suggestions = [
          {
            ...
            reason: `High volatility (${marketVolatility.toFixed(
              1
            )}%) detected - optimal for quick profit scalping`,
          ...
          },
          {
            ...
            reason: `Price swings of ${btcVolatility.toFixed(
              1
            )}% create excellent grid trading opportunities`,
        ...
        suggestions = [
          {
            ...
            reason: `Moderate volatility (${marketVolatility.toFixed(
              1
            )}%) perfect for swing setups`,
        ...
        suggestions = [
          {
            ...
            reason: `Low volatility (${marketVolatility.toFixed(
              1
            )}%) excellent for long-term building`,
      ...
      res.json({
        ...
        marketConditions: {
          ...
          trend: (btc?.percentage || 0) > 0 ? "bullish" : "bearish",
        },
  ...
  app.get("/api/order-book", async (req, res) => {
    try {
      ...
      try {
        const { bids, asks } = await exchangeService.getOrderBookData(
          exchangeName,
          symbol,
          limit
        );
        ...
        return res.status(503).json({
          ...
          message:
            ...
            "Unable to fetch order book data from exchange",
        });
      }
    } catch (error: any) {
  ...
  app.get("/api/trades", async (req, res) => {
    try {
      ...
      try {
        const trades = await exchangeService.getRecentTrades(
          exchangeName,
          symbol,
          limit
        );
  ...
  app.get("/api/portfolio/risk", authenticate, async (req: any, res) => {
    try {
      ...
      for (const pos of positions) {
        const currentPrice =
          priceMap[pos.symbol] ||
          parseFloat(pos.currentPrice as any) ||
          parseFloat(pos.entryPrice as any);
      ...
      const valueAtRisk = totalExposure * 0.02; // demo VaR
      const sharpeRatio = totalExposure !== 0 ? (totalPnl / totalExposure) * 10 : 0;
        ...
        try {
          ...
          if (!exchangeBalances || exchangeBalances.length === 0) {
            return res.status(400).json({
              ...
              message:
                "No real exchange balance found for ${exchange}. Connect your exchange API keys first.",
            });
          }
          ...
          const minimumBalance =
            type === "oco" && ocoLimitPrice
              ? orderValue * parseFloat(ocoLimitPrice)
              : orderValue * 100;

          const relevantBalance = quoteBalance ? quoteBalance.free : baseBalance ? baseBalance.free : 0;
          if (relevantBalance < minimumBalance * 0.1) {
            return res.status(400).json({
              error: "INSUFFICIENT_BALANCE",
              message: `Insufficient real balance on ${exchange}. Required: ~${(
                minimumBalance * 0.1
              ).toFixed(4)} ${quoteCurrency || baseCurrency}, Available: ${relevantBalance.toFixed(
                4
              )}`,
            });
          }
  ...
  // AI Trading API
  app.get(
    "/api/ai/strategy-recommendations/:symbol",
    authenticate,
    async (req: any, res) => {
      try {
        const { symbol } = req.params;
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const signal = await aiTradingService.generateTradingSignal(
          userId,
          botId,
          symbol,
          exchange
  ...
  app.get(
    "/api/ai/market-analysis/:symbol/:exchange",
    authenticate,
    async (req: any, res) => {
      try {
    ...
    async (req: any, res) => {
      try {
        ...
        if (!symbol || !side || !type || !quantity || !exchange) {
          return res
            .status(400)
            .json({ error: "Missing required fields (symbol, side, type, quantity, exchange)" });
        }
        ...
        // Verify real exchange balances before executing trade
        try {
          const exchangeBalances = await exchangeService.getBalance(userId, exchange);
          if (!exchangeBalances || exchangeBalances.length === 0) {
            return res.status(400).json({
          ...
          if (side.toLowerCase() === "buy") {
            if (!quoteBalance || quoteBalance.free <= 0) {
              return res.status(400).json({
                error: "INSUFFICIENT_BALANCE",
                message: `Insufficient ${quoteCurrency} balance on ${exchange}. Available: ${
                  quoteBalance?.free || 0
                }`,
              });
            }
            ...
            const requiredAmount =
              type === "market"
                ? tradeQuantity * 50000
                : tradeQuantity * parseFloat(price || "50000");
            if (quoteBalance.free < requiredAmount) {
              return res.status(400).json({
                error: "INSUFFICIENT_BALANCE",
                message: `Insufficient ${quoteCurrency} balance. Required: ${requiredAmount.toFixed(
                  4
                )}, Available: ${quoteBalance.free.toFixed(4)}`,
              });
            }
            ...
            if (!baseBalance || baseBalance.free <= 0) {
              return res.status(400).json({
                error: "INSUFFICIENT_BALANCE",
                message: `Insufficient ${baseCurrency} balance on ${exchange}. Available: ${
                  baseBalance?.free || 0
                }`,
              });
            }
          ...
          const executedOrder = await exchangeService.placeOrder(
            userId,
            exchange,
            orderParams
          );
...
*/
  ...
  // SAFE for anonymous calls now (no 401). If unauthenticated, returns inactive.
  app.get("/api/subscription/status", authenticateOptional, async (req: any, res) => {
    try {
      if (!req.user) {
        return res.json({ isActive: false, message: "Not logged in" });
      }
      const userId = req.user.id;
  ...
  // verify purchase with IAP services
  app.post("/api/subscription/verify", authenticate, async (req: any, res) => {
    ...
    try {
      ...
      if (provider === "huawei" && purchaseToken) {
        ...
        return res
          .status(400)
          .json({ error: "Invalid provider or missing purchase token/id" });
      }
...
function generateChartData(currentPrice: number, change24h: number, timeframe: string) {
  ...
  switch (timeframe) {
    case "1h":
      intervals = 60; // 1m points
      intervalMs = 60 * 1000;
      break;
    case "4h":
      intervals = 48; // 5m points
      intervalMs = 5 * 60 * 1000;
      break;
    case "1d":
      intervals = 24; // 1h points
      intervalMs = 60 * 60 * 1000;
      break;
    case "7d":
      intervals = 168; // 1h points
      intervalMs = 60 * 60 * 1000;
      break;
    case "30d":
      intervals = 30; // 1d points
  ...
  const priceVolatility = Math.abs(change24h) / 100;
