...
import { apiRequest } from "../lib/auth"; // or "@/lib/auth" if you use the @/* alias
  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    try {
      ...
      if ((result as any)?.token) {
        ...
        // fallback: immediately try login
        const loginResp = await apiRequest("/api/auth/login", "POST", {
          email: data.email,
          password: data.password,
        });
        ...
        if ((loginResp as any)?.token) {
          ...
          toast({
            ...
            description: JSON.stringify(loginResp),
          });
        }
      }
    } catch (error: any) {
